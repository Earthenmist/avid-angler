...
local function VenomAlertTooltip()
    local name = AvidAngler:SafeCall(C_Item and C_Item.GetItemNameByID, nil, 244790)
    if AvidAngler:IsSecretValue(name) or type(name) ~= "string" then name = "The Coiled Huntress" end
    return L["VENOM_ALERT_TOOLTIP"]:format(name)
end
...
function SettingsScreen:CreateCastingPanel(content)
    ...
    local version = C_AddOns.GetAddOnMetadata("AvidAngler", "Version") or ""
    self.settingsVersion = NewLabel(panel, "- v" .. version, Theme.font.heading, "LEFT", self.settingsHeading, "RIGHT", 8, 0)
    self.settingsVersion:SetTextColor(unpack(Theme.color.textSecondary))
    self.interactNote = NewLabel(panel, L["SETTINGS_INTERACT_NOTE"], Theme.font.muted, "TOPLEFT", panel, "TOPLEFT", 0, -26)
    self.interactNote:SetPoint("TOPRIGHT", panel, "TOPRIGHT", -12, -26)
    self.interactNote:SetJustifyH("LEFT")
    ...
    self.autoLootButton:SetWidth(110)
    ...
    self.uiScaleLabel = NewLabel(self.inputPanel, L["UI_SCALE_LABEL"], Theme.font.body, "TOPLEFT", self.inputPanel, "TOPLEFT", 12, -122)
    self.uiScaleLabel:SetWidth(62)
    self.uiScaleDown = Theme:CreateButton(self.inputPanel, "-")
    self.uiScaleDown:SetSize(24, 26)
    self.uiScaleDown:SetPoint("TOPLEFT", self.inputPanel, "TOPLEFT", 78, -116)
    self.uiScaleValue = NewLabel(self.inputPanel, "", Theme.font.body, "LEFT", self.uiScaleDown, "RIGHT", 4, 0)
    self.uiScaleValue:SetWidth(42)
    self.uiScaleValue:SetJustifyH("CENTER")
    self.uiScaleUp = Theme:CreateButton(self.inputPanel, "+")
    self.uiScaleUp:SetSize(24, 26)
    self.uiScaleUp:SetPoint("LEFT", self.uiScaleValue, "RIGHT", 4, 0)
    for _, control in ipairs({self.uiScaleDown, self.uiScaleUp}) do
        AttachTooltip(control, L["UI_SCALE_LABEL"], L["UI_SCALE_TOOLTIP"])
    end
    local function ChangeScale(delta)
        self.owner:SetUIScale(self.owner:GetUIScale() + delta)
        self:Refresh()
    end
    self.uiScaleDown:SetScript("OnClick", function() ChangeScale(-0.05) end)
    self.uiScaleUp:SetScript("OnClick", function() ChangeScale(0.05) end)

    self.audioFocusButton = Theme:CreateButton(self.inputPanel, "")
    self.audioFocusButton:SetPoint("TOPLEFT", self.inputPanel, "TOPLEFT", 190, -116)
    self.audioFocusButton:SetWidth(130)
    ...
    self.venomAlertLabel = NewLabel(self.inputPanel, L["VENOM_ALERT_SETTING"], Theme.font.body, "LEFT", softIconButton, "RIGHT", 12, 0)
    self.venomAlertLabel:SetWidth(math.min(118, self.venomAlertLabel:GetStringWidth()))
    self.venomAlertBox = Theme:CreateEditBox(self.inputPanel, 64)
    self.venomAlertBox:SetPoint("LEFT", self.venomAlertLabel, "RIGHT", 6, 0)
    self.venomAlertBox:SetHeight(22)
    self.venomAlertBox:SetMaxLetters(10)
    self.venomAlertSave = Theme:CreateButton(self.inputPanel, L["SAVE_KEY_LABEL"])
    self.venomAlertSave:SetSize(70, 22)
    self.venomAlertSave:SetPoint("LEFT", self.venomAlertBox, "RIGHT", 6, 0)
    AttachTooltip(self.venomAlertBox, L["VENOM_ALERT_SETTING"], VenomAlertTooltip)
    AttachTooltip(self.venomAlertSave, L["VENOM_ALERT_SETTING"], VenomAlertTooltip)
    local function SaveVenomAlert()
        local alert = AvidAngler.Modules.VenomAlert
        if alert and alert:SetThreshold(self.venomAlertBox:GetText()) then
            self.venomAlertBox:ClearFocus()
            self:Refresh()
            alert:Update()
        else
            UIErrorsFrame:AddMessage(L["VENOM_ALERT_INVALID"], 1, 0.3, 0.3)
        end
    end
    self.venomAlertSave:SetScript("OnClick", SaveVenomAlert)
    self.venomAlertBox:SetScript("OnEnterPressed", SaveVenomAlert)
    ...
    self.keyBindingValue = NewLabel(self.inputPreviewPanel, "", Theme.font.body, "LEFT", self.keyLabel, "RIGHT", Theme.layout.gutter, 0)
    self.keyBindingValue:SetWidth(140)
    self.keySaveButton = Theme:CreateButton(self.inputPreviewPanel, L["KEY_BINDINGS_OPEN"])
    self.keySaveButton:SetPoint("LEFT", self.keyBindingValue, "RIGHT", Theme.layout.gutter, 0)
    self.keySaveButton:SetWidth(190)
    AttachTooltip(self.keySaveButton, L["KEY_BINDINGS_OPEN"], L["KEY_BINDINGS_HELP"])
...
function SettingsScreen:RefreshCasting()
    ...
    if castingEnabled then
        ...
        self.keyBindingValue:SetText(casting:GetBindingDisplay())
...
function SettingsScreen:RefreshLocaleText()
    ...
    self.interactNote:SetText(L["SETTINGS_INTERACT_NOTE"])
    self.uiScaleLabel:SetText(L["UI_SCALE_LABEL"])
    local scale = self.owner:GetUIScale()
    self.uiScaleValue:SetText(string.format("%d%%", math.floor(scale * 100 + 0.5)))
    self.uiScaleDown:SetEnabled(scale > 0.6001)
    self.uiScaleUp:SetEnabled(scale < 1.2999)
    for _, control in ipairs({self.uiScaleDown, self.uiScaleUp}) do
        control.tooltipTitle = L["UI_SCALE_LABEL"]
        control.tooltipBody = L["UI_SCALE_TOOLTIP"]
    end
    ...
    self.keySaveButton.text:SetText(L["KEY_BINDINGS_OPEN"])
    self.keySaveButton.tooltipTitle = L["KEY_BINDINGS_OPEN"]
    self.keySaveButton.tooltipBody = L["KEY_BINDINGS_HELP"]
...
function SettingsScreen:Refresh()
    ...
    self:RefreshCasting()
    self.venomAlertLabel:SetText(L["VENOM_ALERT_SETTING"])
    self.venomAlertLabel:SetWidth(0)
    self.venomAlertLabel:SetWidth(math.min(118, self.venomAlertLabel:GetStringWidth()))
    self.venomAlertSave.text:SetText(L["SAVE_KEY_LABEL"])
    for _, control in ipairs({ self.venomAlertBox, self.venomAlertSave }) do
        control.tooltipTitle, control.tooltipBody = L["VENOM_ALERT_SETTING"], VenomAlertTooltip
    end
    if not self.venomAlertBox:HasFocus() then
        local settings = AvidAngler.DataLayer:GetVenomAlertSettings()
        local value = settings and settings.threshold
        self.venomAlertBox:SetText(value and value > 0 and tostring(value) or "")
    end
end
...
function SettingsScreen:WireScripts()
    ...
    self.keySaveButton:SetScript("OnClick", AvidAngler.UI.OpenFishingKeybindings)
