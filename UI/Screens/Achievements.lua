...
function Screen:CreateGuide()
    ...
    local mainEscapeIndex
    dialog:SetScript("OnShow", function()
        for index = #UISpecialFrames, 1, -1 do
            if UISpecialFrames[index] == "AvidAnglerMainFrame" then
                mainEscapeIndex = index
                table.remove(UISpecialFrames, index)
            end
        end
    end)
    dialog:SetScript("OnHide", function()
        if not mainEscapeIndex then return end
        -- CloseSpecialWindows visits every registered frame in one pass.
        -- Restore the parent only after that Escape pass has finished.
        C_Timer.After(0, function()
            if dialog:IsShown() or not mainEscapeIndex then return end
            for _, name in ipairs(UISpecialFrames) do
                if name == "AvidAnglerMainFrame" then mainEscapeIndex = nil; return end
            end
            table.insert(UISpecialFrames, math.min(mainEscapeIndex, #UISpecialFrames + 1), "AvidAnglerMainFrame")
            mainEscapeIndex = nil
        end)
    end)
