...
function SetupWizard:Initialize()
    ...
    self.keyBox = Theme:CreateEditBox(frame, 160)
    self.keyBox:SetPoint("LEFT", self.keyLabel, "RIGHT", 8, 0)
    self.keyBox:SetText("CTRL-F")
    self.keyLabel:Hide()
    self.keyBox:Hide()
...
function SetupWizard:Finish()
    ApplySelections()
    local casting = GetCasting()
    if casting and casting.SetOneKey then
        casting:SetOneKey(self.keyBox:GetText())
    end
    AvidAngler:SetSetupWizardComplete(true)
    self:Hide()
...
function SetupWizard:Show(force)
    ...
    state.page = 1
    if state.oneKey and state.oneKey ~= "" then
        self.keyBox:SetText(state.oneKey)
    end
    self:Refresh()
    self.frame:Show()
end
