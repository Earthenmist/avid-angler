...
local sessionEarnings = CreateFrame("Frame", nil, trackingPanel)
sessionEarnings:SetPoint("BOTTOMLEFT", trackingNextButton, "BOTTOMRIGHT", 16, 0)
sessionEarnings:SetPoint("BOTTOMRIGHT", trackingPanel, "BOTTOMRIGHT", -4, 0)
sessionEarnings:SetHeight(32)
sessionEarnings:EnableMouse(true)
sessionEarnings.total = NewLabel(sessionEarnings, "", Theme.font.body, "TOPRIGHT", sessionEarnings, "TOPRIGHT", 0, 0)
sessionEarnings.total:SetPoint("LEFT", sessionEarnings, "LEFT", 0, 0)
sessionEarnings.total:SetJustifyH("RIGHT")
sessionEarnings.sources = NewLabel(sessionEarnings, "", Theme.font.muted, "TOPRIGHT", sessionEarnings.total, "BOTTOMRIGHT", 0, -3)
sessionEarnings.sources:SetPoint("LEFT", sessionEarnings, "LEFT", 0, 0)
sessionEarnings.sources:SetJustifyH("RIGHT")
sessionEarnings:SetScript("OnEnter", function(self)
    GameTooltip:SetOwner(self, "ANCHOR_TOP")
    GameTooltip:SetText(L["SESSION_VALUE_TITLE"])
    GameTooltip:AddLine(L["SESSION_VALUE_TOOLTIP"], 1, 1, 1, true)
    GameTooltip:Show()
end)
sessionEarnings:SetScript("OnLeave", function() GameTooltip:Hide() end)

function MainWindow:RefreshSessionValue()
    local tracking = AvidAngler.Modules.Tracking
    local visible = frame:IsShown() and activeTab == "tracking" and IsModuleEnabled("tracking")
        and not IsRuntimeDisabled() and not InCombatLockdown()
        and tracking and tracking.SessionValue
    sessionEarnings:SetShown(visible and true or false)
    if visible then
        local total, sources = tracking.SessionValue:GetDisplayText()
        sessionEarnings.total:SetText(total)
        sessionEarnings.sources:SetText(sources)
    end
end
...
frame:SetScript("OnUpdate", function(_, elapsed)
    ...
    refreshElapsed = 0
    if frame:IsShown() then
        MainWindow:RefreshFishingStatus()
        MainWindow:RefreshSessionValue()
    end
