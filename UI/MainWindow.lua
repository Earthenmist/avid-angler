...
function ExpansionSkill:ProfessionIDMatches(info, professionIDs)
    local professionID = type(info) == "table" and tonumber(info.professionID)
...
function ExpansionSkill:IsSkillInfoLearned(info)
    ...
    if info.learned == false or info.isLearned == false or info.unlocked == false or info.isUnlocked == false then
        return false
    end
    if info.learned or info.isLearned or info.unlocked or info.isUnlocked then
        return true
    end
    local skillLevel = tonumber(info.skillLevel or info.currentLevel or info.rank or info.numSkillUps)
    return skillLevel and skillLevel > 0 or false
end
...
function ExpansionSkill:StatesEqual(left, right)
    for expansionKey in pairs(EXPANSION_NAV_KEYS) do
        if (left and left[expansionKey] or false) ~= (right and right[expansionKey] or false) then
            return false
        end
    end
    return true
...
function ExpansionSkill:CacheChildSkillStates(children, catalog)
    ...
    for _, tab in ipairs(PRIMARY_NAV) do
        if tab.expansion then
            states[tab.key] = false
        end
    end

    for _, child in ipairs(children or {}) do
        if self:IsSkillInfoLearned(child) then
            for expansionKey in pairs(states) do
                local skillNames = catalog.GetSkillLineNames and catalog:GetSkillLineNames(expansionKey) or {}
                local professionIDs = catalog.GetProfessionIDs and catalog:GetProfessionIDs(expansionKey) or {}
                if self:ProfessionIDMatches(child, professionIDs) or self:SkillInfoMatches(child, skillNames) then
                    states[expansionKey] = true
                end
            end
        end
    end

    local changed = not self:StatesEqual(self.cachedStates, states)
...
function ExpansionSkill:LoadSavedStates()
    ...
    local states = {}
    for _, tab in ipairs(PRIMARY_NAV) do
        if tab.expansion and snapshot.expansions[tab.key] ~= nil then
            states[tab.key] = snapshot.expansions[tab.key] and true or false
        end
    end
    if next(states) then
        self.cachedStates = states
...
function ExpansionSkill:RefreshFromJournal()
    ...
    if not catalog or not tradeSkill or type(tradeSkill.GetChildProfessionInfos) ~= "function" then
        return nil
    end

    local children = AvidAngler.SafeCall and AvidAngler:SafeCall(tradeSkill.GetChildProfessionInfos, nil, self.fishingSkillLineID) or tradeSkill.GetChildProfessionInfos(self.fishingSkillLineID)
...
function ExpansionSkill:CharacterHasFishingSkill(expansionKey)
    ...
    if liveStates then
        return liveStates[expansionKey] == true
    end
