...
*   Extra toy slots skip toys with a recognised active buff. Other toys retain their configured repeat delay; not every toy uses the same spell ID for its use and its buff.
*   Unusable selected fishing items are skipped with a notice. Pole lures target the equipped profession rod, or a main-hand fishing pole if no profession rod is equipped; unavailable rod or enchant data safely skips application.
*   Track catches, session history, fish, zones, sources, rates and dashboard summaries.
*   See estimated session catch value and gold/hour, with optional Auctionator or TSM pricing and vendor fallback.
*   View each expansion's fishing skill and bonuses, with saved levels when live data is unavailable.
*   Track Venom on The Coiled Huntress in the fishing-tool slot and optionally receive conversion reminders.
...
The Dashboard footer shows estimated value for the entire current session,
regardless of history filters. Each item uses Auctionator's cached price first,
then TSM's DBMarket, then its vendor sell price. Neither pricing addon is
required. Bound items use vendor value; items without a known price are counted
separately and excluded from the estimate. Prices refresh while the display is
open, so the estimate can change as pricing data becomes available.

This is gross catch value before auction fees and fishing-supply costs, not
recorded sales or profit. Gold/hour appears after one minute and includes idle
time since login/reload or **Reset Session**, refreshing every 30 seconds.
Session values reset on reload;
saved catch history remains intact. Hover over the estimate for an explanation.
...
*   Assign your fishing shortcut under **Game → Keybindings → Avid Angler**. The **Keybindings** button in Avid Angler Settings opens this page; Blizzard handles conflicts and lets you assign two keys. Existing shortcuts migrate only when they are not assigned to another action.
...
| <code>/aaone key CTRL-F</code> |Assign Ctrl+F if it is free; use Blizzard Keybindings to resolve conflicts   |
...
For Venom reminders, set **Venom alert at** in **Settings → General settings**.
The value is remembered per character; blank or **0** keeps reminders off.
Closing a reminder silences it until another 25% of your threshold accumulates
or you next log in. Converting below the threshold resets the reminder.
Tracking must be enabled and The Coiled Huntress equipped in the fishing-tool
slot. Reminders work with the main window closed and pause during combat or
protected content.
...
The minimap launcher includes LibDBIcon and its dependencies for compatibility
with minimap button collectors. No separate library download is needed.
