...
local EXCLUDED_LURE_ITEM_IDS = {
    [262649] = true, -- An Angler's Deep Dive grants permanent profession skill.
    [262787] = true, -- Dredged Journal Entry grants permanent profession skill.
...
-- Native bindings remain inert when the active fishing input is disabled.
do
    local nativeBindingButton = CreateFrame("Button", "AvidAnglerFishingBindingButton", UIParent, "SecureActionButtonTemplate")
    nativeBindingButton:RegisterForClicks("AnyDown", "AnyUp")
    nativeBindingButton:SetAttribute("type", "macro")
    nativeBindingButton:SetAttribute("macrotext", "")
    nativeBindingButton:SetSize(1, 1)
    nativeBindingButton:SetPoint("TOPLEFT", UIParent, "TOPLEFT", -10, 10)
    nativeBindingButton:EnableMouse(false)
    nativeBindingButton:SetAlpha(0)
    nativeBindingButton:Show()
end

-- Both templates are needed on one frame: SecureActionButtonTemplate for
-- the protected macro click, BackdropTemplate for the shared theme.
...
castButton:SetScript("OnEnter", function(self)
    visualHover = true
    castCloseButton:SetFrameStrata(self:GetFrameStrata())
    castCloseButton:SetFrameLevel(self:GetFrameLevel() + 10)
...
-- Keep this independent of the secure action button, but above its hit area.
castCloseButton:SetFrameStrata(castButton:GetFrameStrata())
castCloseButton:SetFrameLevel(castButton:GetFrameLevel() + 10)
castCloseButton:SetPoint("TOPRIGHT", castButton, "TOPRIGHT", 2, 2)
castCloseButton:SetSize(18, 18)
castCloseButton:Hide()
...
local function ApplyButtonSize()
    ...
    castCloseButton:SetPoint("TOPRIGHT", castButton, "TOPRIGHT", 2, 2)
    local closeSize = math.max(16, math.min(24, buttonSize * 0.3))
    castCloseButton:SetSize(closeSize, closeSize)
    castCloseButton:SetScale(castButton:GetEffectiveScale() / UIParent:GetEffectiveScale())
end
...
local function CheckChoiceCriteria(choice)
    if choice and choice.source == "bag" then
        if InCombatLockdown() or AvidAngler:IsRuntimeDisabled() then return false end
        return AvidAngler:SafeCall(C_PlayerInfo and C_PlayerInfo.CanUseItem, nil, choice.itemID) == true
            and AvidAngler:SafeCall(C_Item and C_Item.IsUsableItem, nil, choice.itemID) == true
    end
...
local function CategorizeBagItem(itemID, name, spellName)
    ...
    -- A Fishing use-spell alone also matches permanent profession skill items.
    ...
    for bag = 0, NUM_BAG_SLOTS do
        ...
        for slot = 1, slots do
            ...
            if itemID then
                ...
                local key = (usable or GetLureCatalogEntry(itemID)) and CategorizeBagItem(itemID, name, spellName)
                if key then
...
local function SelectFirstChoiceIfNeeded(key)
    ...
    if (key == "lure" or key == "ward") then
        local catalog = AvidAngler.Data and AvidAngler.Data.LureCatalog
        local knownSelection = picker.manualChoice and picker.source == "bag" and catalog
            and catalog:GetPickerKey(picker.itemID) == key
        if not picker.manualChoice or (not choice and not knownSelection) then
            picker.source = nil
            picker.itemID = nil
...
local function RefreshActionChoices()
    ...
    ScanBagChoices()
    ScanToyChoices()
    -- Random is a saved preference, even when a rescan temporarily finds
    -- fewer usable choices. Never replace it with the first remaining item.
    for _, key in ipairs({ "bobber", "toy" }) do
        local picker = pickerSettings[key]
        if #(actionChoices[key] or {}) > 1 or (picker and picker.source == "random") then
            AddRandomChoice(key)
        end
    end
    for _, key in ipairs(PICKER_KEYS) do
...
function Helpers.GetFishingPoleSlot()
    if InCombatLockdown() or AvidAngler:IsRuntimeDisabled() then return nil end
    if type(GetInventoryItemID) ~= "function" then return nil end
    local ok, tool = pcall(GetInventoryItemID, "player", 28)
    if not ok or AvidAngler:IsSecretValue(tool) then return nil end
    if type(tool) == "number" and tool > 0 then return 28 end
    local mainHand = AvidAngler:SafeCall(GetInventoryItemID, nil, "player", 16)
    if not mainHand or not C_Item or not C_Item.GetItemInfoInstant then return nil end
    local ok, _, _, _, _, _, classID, subclassID = pcall(C_Item.GetItemInfoInstant, mainHand)
    if ok and AvidAngler:SafeNumber(classID, nil) == 2
        and AvidAngler:SafeNumber(subclassID, nil) == 20 then return 16 end
end

function Helpers.GetPoleEnhancementActive()
    local slot = Helpers.GetFishingPoleSlot()
    ...
    if C_PaperDollInfo and C_PaperDollInfo.GetTemporaryEnchantmentInfo then
        ...
        if not ok or AvidAngler:IsSecretValue(info) then return nil end
        if info == nil then return false end
        if type(info) ~= "table" or AvidAngler:IsSecretValue(info.enchantID) then return nil end
        if type(info.enchantID) ~= "number" or info.enchantID ~= info.enchantID then return nil end
        return info.enchantID > 0 and true or nil
    end
    if slot == 16 and type(GetWeaponEnchantInfo) == "function" then
        local ok, enchanted = pcall(GetWeaponEnchantInfo)
        if ok and not AvidAngler:IsSecretValue(enchanted) then return enchanted == true end
    end
end
...
local function IsChoiceAuraActive(choice)
    ...
    if (choice.key == "lure" or choice.key == "ward") then
        return HasKnownPickerAuraActive(choice.key)
            ...
            or (choice.poleEnhancement and Helpers.GetPoleEnhancementActive() == true)
    end
...
local function GetChoiceMacro(choice)
    ...
    local macro = "/use item:" .. tostring(choice.itemID)
    if choice.poleEnhancement then
        local slot = Helpers.GetFishingPoleSlot()
        if not slot then return nil end
        macro = macro .. "\n/use " .. slot .. "\n/stopspelltarget"
    end
    return Helpers.PrefixDismountMacro(macro)
end
...
local function IsChoiceUsable(choice)
    ...
    if choice.source == "bag" then
        local count = AvidAngler:SafeNumber(AvidAngler:SafeCall(C_Item.GetItemCount, nil, choice.itemID), nil)
        if not count or count <= 0 then return false end
    end
    if choice.poleEnhancement and Helpers.GetPoleEnhancementActive() ~= false then return false end
...
local function GetFixedChoiceStatus(key, picker, choice)
    ...
    if choice.poleEnhancement and not Helpers.GetFishingPoleSlot() then return "rod", 0 end
    if choice.poleEnhancement and Helpers.GetPoleEnhancementActive() == nil then return "blocked", 0 end
...
local function FindReadyPickerAction()
    Helpers.pickerWarnings = Helpers.pickerWarnings or {}
    for _, key in ipairs(GetPickerOrder()) do
        ...
        local warning
        if picker and picker.enabled and choice and choice.source == "bag" and not IsChoiceAuraActive(choice) then
            if not CheckChoiceCriteria(choice) then
                warning = L["ACTION_PICKER_UNUSABLE"]
            elseif choice.poleEnhancement and not Helpers.GetFishingPoleSlot() then
                warning = L["ACTION_PICKER_STATUS_ROD"]
            elseif choice.poleEnhancement and Helpers.GetPoleEnhancementActive() == nil then
                warning = L["ACTION_PICKER_ENCHANT_UNKNOWN"]
            end
        end
        local warningID = warning and (choice.itemID .. ":" .. warning)
        if warningID and Helpers.pickerWarnings[key] ~= warningID then
            AvidAngler:Print(L["ACTION_PICKER_SKIPPED"]:format(choice.name, warning))
        end
        Helpers.pickerWarnings[key] = warningID
...
function Helpers.IsActionSlotAuraActive(slot)
    if not slot or slot.kind ~= "toy" or not slot.itemID then return false end
    if InCombatLockdown() or AvidAngler:IsRuntimeDisabled() then return false end
    -- Prefer known toy aura IDs when the use-spell and resulting buff differ.
    for _, toy in ipairs(FISHING_TOYS) do
        if toy.itemID == slot.itemID and toy.auraManaged and HasPlayerAura(toy.spellID) then
            return true
        end
    end
    local _, spellID = GetItemSpell(slot.itemID)
    if AvidAngler:IsSecretValue(spellID) or type(spellID) ~= "number" then return false end
...
local function FindReadyActionSlot()
    for index = 1, ACTION_SLOT_COUNT do
        ...
        if slot and slot.enabled and BuildActionSlotMacro(slot) ~= "" and GetSlotRemaining(slot) <= 0 and not Helpers.IsActionSlotAuraActive(slot) then
            return index, slot
        end
    end
end
...
Helpers.FISHING_BINDING_ACTION = "CLICK AvidAnglerFishingBindingButton:LeftButton"
function Helpers.GetFishingBindingKeys()
    if not GetBindingKey then return {} end
    local keys = { GetBindingKey(Helpers.FISHING_BINDING_ACTION) }
    local result = {}
    for _, key in ipairs(keys) do
        if not AvidAngler:IsSecretValue(key) and type(key) == "string" and key ~= "" then result[#result + 1] = key end
    end
    return result
end

function Helpers.IsFishingKeyHeld(key)
    if not key or key == "" then return false end
    key = key:gsub("ALT%-", ""):gsub("CTRL%-", ""):gsub("SHIFT%-", "")
    local mouseIndex = key:match("^BUTTON(%d+)$")
    if mouseIndex then
        local buttons = { "LeftButton", "RightButton", "MiddleButton", "Button4", "Button5" }
        return IsMouseButtonDown and IsMouseButtonDown(buttons[tonumber(mouseIndex)] or ("Button" .. mouseIndex))
    end
    if key == "MOUSEWHEELUP" or key == "MOUSEWHEELDOWN" then return false end
    return IsKeyDown(key)
end

function Helpers.AnyFishingKeyHeld()
    for _, key in ipairs(Helpers.GetFishingBindingKeys()) do
        if Helpers.IsFishingKeyHeld(key) then return true end
    end
    return false
end
...
local function ApplyFishingBindings()
    ...
    if inputMode == INPUT_MODE_ONE_KEY and oneKeyEnabled then
        for _, key in ipairs(Helpers.GetFishingBindingKeys()) do
            if Helpers.IsFishingKeyHeld(key) then
                pendingBindingUpdate = true
            else
                SetFishingActionBinding(key, "LeftButton")
            end
        end
    end
...
-- Reinstall only after release; rebinding during the original press can
-- turn its key-up into a second fishing action.
local bindingPollElapsed = 0
bindingFrame:SetScript("OnUpdate", function(_, elapsed)
    if not pendingBindingUpdate or InCombatLockdown() then return end
    bindingPollElapsed = bindingPollElapsed + elapsed
    if bindingPollElapsed < 0.05 then return end
    bindingPollElapsed = 0
    if not Helpers.AnyFishingKeyHeld() then ApplyFishingBindings() end
end)
...
function Casting:GetOneKey()
    return Helpers.GetFishingBindingKeys()[1] or ""
end
...
function Casting:SetOneKeyEnabled(enabled)
    ...
    AvidAngler:Print((oneKeyEnabled and AvidAngler.L["ONE_KEY_ENABLED"] or AvidAngler.L["ONE_KEY_DISABLED"]):format(Casting:GetBindingDisplay()))
end

function Casting:GetBindingDisplay()
    local keys = Helpers.GetFishingBindingKeys()
    return #keys > 0 and table.concat(keys, " / ") or NOT_BOUND or ""
end

function Casting:SetOneKey(key)
    local normalized = NormalizeKey(key)
    if not normalized then AvidAngler:Print(L["ONE_KEY_USAGE"]); return false end
    if InCombatLockdown() or AvidAngler:IsRuntimeDisabled() then
        AvidAngler:Print(L["ONE_KEY_COMBAT"]); return false
    end
    local action = AvidAngler:SafeCall(GetBindingAction, nil, normalized)
    if AvidAngler:IsSecretValue(action) or type(action) ~= "string" then return false end
    if action ~= "" and action ~= Helpers.FISHING_BINDING_ACTION then
        local name = _G["BINDING_NAME_" .. action] or action
        AvidAngler:Print(L["KEY_BINDINGS_CONFLICT"]:format(normalized, name))
        return false
    end
    local oldKeys = Helpers.GetFishingBindingKeys()
    if not AvidAngler:SafeCall(SetBinding, nil, normalized, Helpers.FISHING_BINDING_ACTION) then return false end
    for _, oldKey in ipairs(oldKeys) do
        if oldKey ~= normalized and GetBindingAction(oldKey) == Helpers.FISHING_BINDING_ACTION then SetBinding(oldKey) end
    end
    SaveBindings(GetCurrentBindingSet())
    oneKey = normalized
    PersistOneKeySettings()
    QueueFishingBindingUpdate()
    AvidAngler:Print(L["ONE_KEY_KEY_SET"]:format(Casting:GetBindingDisplay()))
    return true
end

function Helpers.MigrateFishingBinding()
    local settings = GetCastingSettings()
    if not settings or settings.nativeBindingMigrated or InCombatLockdown() or AvidAngler:IsRuntimeDisabled() then return end
    settings.nativeBindingMigrated = true
    if #Helpers.GetFishingBindingKeys() == 0 and oneKeyEnabled then
        Casting:SetOneKey(oneKey)
    end
end
    ...
    for index = 1, ACTION_SLOT_COUNT do
        ...
        if slot and slot.enabled then
            ...
            if BuildActionSlotMacro(slot) == "" then
                anyEmpty = true
            elseif Helpers.IsActionSlotAuraActive(slot) then
                anyActive = true
            else
...
function Casting:GetPickerChoice(key)
    ...
    local missingChoice
    if not choice and picker and picker.manualChoice and picker.itemID then
        missingChoice = { source = picker.source, itemID = picker.itemID,
            name = GetItemName(picker.itemID, "item:" .. picker.itemID) }
        if picker.enabled and not knownBuffActive then statusKey = "empty" end
    end
    return {
        key = key,
        ...
        count = #choices,
        index = index or 0,
        choice = choice or missingChoice,
...
-- Supply alerts require a complete readable snapshot; unavailable data is not empty stock.
function Casting:GetSupplyAlertState(key)
    if not moduleEnabled or InCombatLockdown() or AvidAngler:IsRuntimeDisabled() then return nil end
    local picker = pickerSettings[key]
    if not picker or not picker.enabled or not picker.manualChoice or not picker.itemID then return false end
    local count = AvidAngler:SafeCall(C_Item and C_Item.GetItemCount, nil, picker.itemID, false, false, false, false)
    count = AvidAngler:SafeNumber(count, nil)
    if count == nil then return nil end
    local catalog = AvidAngler.Data and AvidAngler.Data.LureCatalog
    if not catalog or not C_UnitAuras or not C_UnitAuras.GetAuraDataByIndex then return nil end
    local active = false
    for index = 1, 255 do
        local ok, aura = pcall(C_UnitAuras.GetAuraDataByIndex, "player", index, "HELPFUL")
        if not ok or AvidAngler:IsSecretValue(aura) then return nil end
        if not aura then break end
        local spellID = AvidAngler:SafeNumber(aura.spellId, nil)
        if not spellID then return nil end
        if catalog:IsPickerBuff(key, spellID) then active = true; break end
        if index == 255 then return nil end
    end
    if key == "lure" and catalog:IsPoleEnhancement(picker.itemID) then
        local enchanted = Helpers.GetPoleEnhancementActive()
        if enchanted == nil then return nil end
        active = active or enchanted == true
    end
    return { itemID = picker.itemID, count = count, active = active }
end
...
local keyBox = controlFrame:CreateFontString(nil, "ARTWORK")
keyBox:SetFontObject(Theme.font.body)
keyBox:SetPoint("LEFT", keyLabel, "RIGHT", Theme.layout.gutter, 0)
keyBox:SetWidth(120)
local saveKeyButton = Theme:CreateButton(controlFrame, L["KEY_BINDINGS_OPEN"])
saveKeyButton:SetPoint("LEFT", keyBox, "RIGHT", Theme.layout.gutter, 0)
saveKeyButton:SetWidth(160)
...
function Casting:RefreshControlPanel()
    ...
    keyBox:SetText(self:GetBindingDisplay())
end
...
saveKeyButton:SetScript("OnClick", function()
    AvidAngler.UI.OpenFishingKeybindings()
end)
...
SlashCmdList["AVIDANGLERONEKEY"] = function(msg)
    ...
    elseif command == "status" or command == "" then
        if oneKeyEnabled then
            AvidAngler:Print(AvidAngler.L["ONE_KEY_STATUS_ENABLED"]:format(Casting:GetBindingDisplay()))
        else
