...
local PICKER_KEYS = { "oversized", "bobber", "lure", "toy" }
local WATER_PICKER_KEYS = { "toy", "oversized", "bobber", "lure" }
...
local actionSlots = {}
local actionChoices = { lure = {}, oversized = {}, bobber = {}, toy = {} }
    ...
    for bag = 0, NUM_BAG_SLOTS do
        ...
        for slot = 1, slots do
            ...
            if itemID then
                ...
                if key then
                    local lureEntry = key == "lure" and GetLureCatalogEntry(itemID) or nil
...
local function RefreshActionChoices()
    actionChoices = { lure = {}, oversized = {}, bobber = {}, toy = {} }
...
local function GetKnownLureAuraActive()
    local catalog = AvidAngler.Data and AvidAngler.Data.LureCatalog
    if not catalog then
        return false
    end
    ...
    for index = 1, 40 do
        ...
        if catalog:IsKnownBuffSpellID(auraSpellID) then
            return true, auraName
        end
        if catalog:IsKnownBuffName(auraName) then
            return true, auraName
        end
    end
    return false
...
local function HasKnownLureAuraActive()
    local active = GetKnownLureAuraActive()
...
local function IsChoiceAuraActive(choice)
    ...
    if choice.key == "lure" then
        return HasKnownLureAuraActive()
...
local function FindReadyPickerAction()
    for _, key in ipairs(GetPickerOrder()) do
        ...
        if key == "lure" and picker and not picker.manualChoice then
            choice = nil
        end
    ...
    for _, key in ipairs(GetPickerOrder()) do
        ...
        queue[#queue + 1] = {
            ...
            detail = state.choice and state.choice.name
                or state.activeName
                or (key == "lure" and state.count > 0 and L["ACTION_PICKER_NONE_SELECTED"])
...
function Casting:GetPickerChoice(key)
    ...
    local knownLureActive, activeLureName
    if key == "lure" then
        knownLureActive, activeLureName = GetKnownLureAuraActive()
    end
    ...
    if picker and picker.enabled then
        if key == "lure" and knownLureActive then
            statusKey, statusRemaining = "active", 0
        elseif key == "lure" and #choices > 0 and not choice then
    ...
    return {
        ...
        activeName = activeLureName and L["ACTION_PICKER_ACTIVE_CHOICE"]:format(activeLureName) or nil,
    }
end
...
function Casting:SelectPickerChoiceByIndex(key, index)
    ...
    picker.manualChoice = key == "lure" and true or picker.manualChoice
    picker.enabled = true
    PersistOneKeySettings()
    QueueFishingBindingUpdate()
end
