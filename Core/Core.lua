...
function AvidAngler.DataLayer:SaveCharacterFishingSkillSnapshot(states)
    ...
    -- Fishing cannot be unlearned; incomplete reads must not erase knowledge.
    local previous = db.skillCache.characters[characterKey]
    snapshot.levels = previous and previous.levels or nil
    for expansionKey, learned in pairs(previous and previous.expansions or {}) do
        if learned == true then snapshot.expansions[expansionKey] = true end
    end
...
function AvidAngler.DataLayer:UpdateCharacterFishingSkillState(expansionKey, learned, source)
    ...
    snapshot.expansions[expansionKey] = snapshot.expansions[expansionKey] == true or learned == true
...
function AvidAngler.DataLayer:GetVenomAlertSettings()
    local key = AvidAngler:GetCurrentCharacterKey()
    if not key then return nil end
    local db = EnsurePersistentTables()
    db.venomAlerts = db.venomAlerts or {}
    db.venomAlerts[key] = db.venomAlerts[key] or {}
    return db.venomAlerts[key]
end

function AvidAngler.DataLayer:SaveCharacterFishingLevel(expansionKey, level)
    local snapshot = self:GetCharacterFishingSkillSnapshot()
    if not snapshot or not snapshot.expansions or snapshot.expansions[expansionKey] ~= true then
        snapshot = self:UpdateCharacterFishingSkillState(expansionKey, true, "skillLevel")
    end
    if not snapshot then return end
    snapshot.levels = snapshot.levels or {}
    local previous = snapshot.levels[expansionKey]
    if previous and previous.rank == level.rank and previous.maximum == level.maximum and previous.bonus == level.bonus then return end
    snapshot.levels[expansionKey] = { rank = level.rank, maximum = level.maximum, bonus = level.bonus }
end
